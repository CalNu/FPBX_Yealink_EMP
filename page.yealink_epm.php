    ...
    if ($enable) {
        ...
        try {
            ...
            if (file_exists($gen_script)) {
                ...
                $generated_path = ($return_var === 0) ? $tar_path_vpnkeys : false;
                if ($return_var !== 0) {
        ...
        if (($generated_path && file_exists($generated_path)) || file_exists($tar_path_vpnkeys)) {
            ...
            echo json_encode(['status' => 'success', 'enabled' => true, 'connected' => false]);
            exit;
        } else {
            echo json_encode([
                'status' => 'error', 
...
// Load UI view template from root module folder
require_once __DIR__ . '/yealink_epm.views.php';